  ...
  async function loadSessions() {
    ...
    try {
      ...
      if (data.length) {
        ...
        const hardcodedById = Object.fromEntries(HARDCODED_SESSIONS.map(s => [s.id, s]))
        const merged = data.map(session => hardcodedById[session.id] || session)
        const seen = new Set(merged.map(s => s.id))
        for (const session of HARDCODED_SESSIONS) {
          if (session.free && !seen.has(session.id)) {
